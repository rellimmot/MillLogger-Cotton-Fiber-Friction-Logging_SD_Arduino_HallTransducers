...
#define LOG_INTERVAL 49 //How long do we want between log intervals (in ms)?
...
void setup(){
  ...
  delay(500); //I put this in here for the SpinLab, so it doesnt start the record trigger when it senses
              //the transient load spike of the vacuum motor starting.
}//END SETUP
...
Serial.println(SensorADC1);
if(SensorADC1 > 50){ //This is the Trigger, so this would set it to log only when the machine
